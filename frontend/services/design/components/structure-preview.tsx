...
export function StructurePreview({ guestId, plan, config, onBack, onGenerated }: StructurePreviewProps) {
  ...
  // Per-slide real overflow, keyed by slide index - null until that
  // slide's own renderer has actually measured itself (see
  // templates/use-balanced-alignment.ts). Falls back to the
  // character-count heuristic below until then, same as the live editor.
  const [renderedOverflow, setRenderedOverflow] = useState<Record<number, boolean | null>>({});
    ...
    <div className="space-y-6">
      <div className="space-y-5">
        {slides.map((slide) => {
          ...
          const measured = renderedOverflow[slide.index] ?? null;
          const fit: ContentFitResult =
            measured === null
              ? limits
                ? evaluateContentFit(limits, slide.headline, slide.body_text)
                : { fits: true, reasons: [] }
              : measured
                ...
                {template ? (
                  <template.Renderer
                    ...
                    onOverflowChange={(overflows) =>
                      setRenderedOverflow((prev) => ({ ...prev, [slide.index]: overflows }))
                    }
                  />
                ) : null}
              </div>
