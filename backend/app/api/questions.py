...
def create_question(
    guest_id: uuid.UUID, question_in: QuestionCreate, db: Session = Depends(get_db)
) -> QuestionResponse:
    try:
...
def get_question(question_id: uuid.UUID, db: Session = Depends(get_db)) -> QuestionResponse:
    try:
...
def update_question(
    question_id: uuid.UUID, question_in: QuestionUpdate, db: Session = Depends(get_db)
) -> QuestionResponse:
    try:
